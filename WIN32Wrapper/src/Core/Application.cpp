...
button_state Application::mouse(BUTTONS button)
{
...
void Application::process_input_state()
{
    ...
    {
        input_state.mouse_state[i].pressed = input_state.new_mouse_state[i];
...
LRESULT CALLBACK Application::windows_event_handler(HWND win, UINT msg, WPARAM w, LPARAM l)
{
    ...
    case WM_MOUSEMOVE:
    {
        ...
        if (GetCursorPos(&p))
        {
            if (ScreenToClient(win, &p))
            {
                Application::App()->set_mouse_position(p.x, p.y);
            }
        }
        return 0;
    }
...
#else
        Application::App()->draw(obj->hdcBuf); // NOTE: USER FUNCTION.
#endif
